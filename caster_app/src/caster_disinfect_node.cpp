...
int main(int argc, char** argv) {
  ...
  geometry_msgs::Pose pose4112, pose4111, pose_west_hall, pose4101, pose_liabray1, pose4086, pose_liabray2, pose_liabray3, pose4102, pose4062, pose_worldcup,
    pose4085, pose4031, pose_community1, pose_community2, pose_community_youth_meeting, pose4021, pose_east_hall, pose4034, pose4051, pose1, pose2, pose3;
  ...
  GetGoalPose("two", private_nh, pose2);
  GetGoalPose("three", private_nh, pose3);

  // set up spinner
  ros::AsyncSpinner spinner(2);
  spinner.start();
  ros::Publisher init_pub = private_nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("initialpose", 1000);
  // ros::WallDuration(1.0).sleep();
  ...
  while(true) {
    ...
    // MoveToGoal(move_base_client, pose1);
    // ROS_INFO("Moveto1");
    // MoveToGoal(move_base_client, pose2);
    // ROS_INFO("Moveto2");
    // MoveToGoal(move_base_client, pose3);
    // ROS_INFO("Moveto3");
